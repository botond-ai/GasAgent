...
class MarkdownRAGChunker:
    ...
    def __init__(self, display_writer: DisplayWriterInterface, chunk_size: int = 800, overlap: int = 150):
        """
    ...
    def chunk_document(self, text: str, filepath: str) -> List[Dict]:
        ...
        self.display_writer.write(f"Generated {len(chunks)} chunks from document {filepath}")
    ...
    def process_file(self, filepath: str) -> List[Dict]:
        try:
            ...
            self.display_writer.write(f"✓ Loaded {os.path.basename(filepath)}")
            return self.chunk_document(text, Path(filepath).name)

        except Exception as e:
            self.display_writer.write(f"✗ Error processing {filepath}: {e}")
            return []
