...
async def lifespan(app: FastAPI):
    ...
    # Initialize Book RAG client
    # Get the absolute path to the PDF file
    import pathlib
    backend_dir = pathlib.Path(__file__).parent.absolute()
    # In Docker, the PDF is mounted at /app/pal_utcai_fiuk.pdf
    # Locally, it's in the parent directory
    docker_pdf_path = pathlib.Path("/app/pal_utcai_fiuk.pdf")
    local_pdf_path = backend_dir.parent / "pal_utcai_fiuk.pdf"
    book_pdf_path = docker_pdf_path if docker_pdf_path.exists() else local_pdf_path
    
    book_client = BookRAGClient(
        pdf_path=str(book_pdf_path),
        openai_api_key=openai_api_key,
        persist_directory=str(backend_dir / "data" / "book_vectordb")
    )
    ...
    # Initialize translator tool (used by BookTool and available standalone)
    translator_tool = TranslatorTool(openai_api_key=openai_api_key)
    
    # Initialize book tool with translator for language detection/translation
    book_tool = BookTool(book_client, translator_tool=translator_tool)
    ...
    photo_memories_folder_id = os.getenv("PCLOUD_PHOTO_MEMORIES_FOLDER_ID")
    ...
    if pcloud_username and pcloud_password:
        try:
            pcloud_client = PCloudClient(
                ...
                photo_memories_folder_id=int(photo_memories_folder_id) if photo_memories_folder_id else None,
...
# Create FastAPI app
app = FastAPI(
    title="AI Agent Demo (Book RAG + Radio tools)",
