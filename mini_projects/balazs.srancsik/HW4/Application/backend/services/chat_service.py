...
class ChatService:
    ...
    async def process_message(self, request: ChatRequest) -> ChatResponse:
        ...
        try:
            ...
            logger.error(f"Failed to load user profile for {user_id}: {e}", exc_info=True)
            # Create a default profile in memory if loading fails
        ...
        try:
            ...
            logger.error(f"Failed to load conversation history for {session_id}: {e}", exc_info=True)
            # Create empty history in memory if loading fails
            history = ConversationHistory(session_id=session_id)
        
        # Build memory context
        memory = self._build_memory(profile, history)
        
        # Add user message to history (non-critical, continue even if fails)
        try:
        ...
        # Run agent
        logger.info(f"About to call agent.run() for user {user_id}")
        try:
            ...
            return ChatResponse(
                final_answer="I apologize, but I encountered an error processing your request. Please try again or rephrase your question.",
                tools_used=[],
                memory_snapshot={
                    "preferences": {"language": profile.language, "default_city": profile.default_city},
                    "workflow_state": WorkflowState().model_dump(),
                    "message_count": len(history.messages)
                },
                logs=[f"Error: {str(e)[:100]}"]
            )
        ...
        # Non-critical operation - continue even if persistence fails
        ...
        try:
            ...
            logger.warning(f"Failed to check profile updates: {e}")
        
        # Reload profile to get any updates
        try:
            updated_profile = await self.user_repo.get_profile(user_id)
        except Exception as e:
            logger.warning(f"Failed to reload profile: {e}")
            updated_profile = profile

        for tc in tools_called:
            if tc.tool_name == "photo_upload":
                logger.info(
                    "tools_used photo_upload: system_message(first 300)=%r detailed_message(first 300)=%r",
                    (tc.system_message or "")[:300],
                    (tc.detailed_message or "")[:300]
                )
        ...
        for tc in tools_called:
            tool_data = {
                ...
                "system_message": (
                    (tc.system_message or "").splitlines()[0]
                    if tc.tool_name == "photo_upload"
                    else tc.system_message
                ),
                "detailed_message": tc.detailed_message
            }
            # For json_creator, include the ticket data in arguments for frontend display
            if tc.tool_name == "json_creator" and tc.result:
                tool_data["arguments"] = {
                    "ticket_data": tc.result.get("ticket_data", {}),
                    "ticket_number": tc.result.get("ticket_number", ""),
                    "json_content": tc.result.get("json_content", "")
                }
    ...
    async def process_message_with_files(
        ...
        """
        Process incoming chat message with attached files.
        
        This method is similar to process_message but also passes file information
        to the agent so the photo_upload tool can access the files.
        ...
        logger.info(f"Processing message with files from user {user_id}, session {session_id}")
        ...
        # Persist all messages from agent
        for msg in agent_messages:
            if isinstance(msg, SystemMessage):
                await self.conversation_repo.add_message(
                    session_id,
        ...
        logger.info(f"Message with files processed successfully for user {user_id}")
        
        return ChatResponse(
            final_answer=final_answer,
            tools_used=tools_used,
            memory_snapshot=memory_snapshot,
            logs=[f"Tools called: {len(tools_called)}", f"Files attached: {len(file_names) if file_names else 0}"]
        )
